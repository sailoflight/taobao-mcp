...
> **JS 控件生成链接**(渠道点击)。按场景选族, 收藏/足迹/购物车三能力全保留(用户会差遣加购/
> 收藏), 足迹优先于收藏(更安全)。

| 场景 | 用哪族 | 依据(矩阵) |
|---|---|---|
| 大批量行为/粗查(A2 游走、搜索、列表标价) | **地址跳转**(裸) | 嵌入式 JSON 不依赖渲染 → 便宜、零配额、零渠道信号; 只需标题/全型号标价时足够 |
| 需要全量渲染的**细查**(详情图文/到手价/评论/推荐) | JS 渠道点击, **足迹优先**(零配额零写入); 收藏仅兜底或用户主动收藏 | 足迹铸造新 mi_id 更安全; 收藏 30/日配额 + 收藏/取关信号, 少用 |
| 用户要收藏商品(整体) | 收藏能力(保留) | 用户主动需求, 非探针用途 |
| 用户要加购(sku+数量+总价优惠)/到手价核对 | 购物车能力(保留; 行自带 skuId+mi_id+upStreamPrice) | 交付能力 + 最高渲染上下文 |
| 单点廉价全量(config mi_id 新鲜时) | 地址跳转+config mi_id | config_detail 实证 raw 32; 会失效/固定足迹 → 仅会话内偶尔用 |
| 会话体检(前端漂移探测) | config_detail 探针 | 每次会话开工先跑 |
...
### 早期"粗查无详情"之谜 — 研究结论(2026-08-20, NOTES.md 原始 recon 实锤)
- **真相: 当时的淘宝 SSR 把详情渲染绑定在 `mi_id` URL 参数上**, 裸 URL(无 mi_id)页面根本没有
  `.desc-root`。NOTES.md:234-253(2026-08-18 现场二分 recon)是第一手证据:
  - 桌面 SSR 直接访问 `item.htm?id=X` → 详情容器 `#description`/`.desc-root` 都不存在,
    滚动 12 秒无任何详情网络请求。
  - 从搜索带跟踪参数点进 → 详情加载; 二分剥离参数 → **`mi_id=<值>` 单独即可触发**;
    编造的 mi_id 不触发(值有语义); 同一个 mi_id 跨多个商品有效(账号/渠道级稳定值)。
  - **裸 URL 页面根本没有 .desc-root**(NOTES.md:253)。
- **referer 说法是当时的误判记录**(desc.py:122 注释): 真正触发条件是 mi_id, 不是 referer。
...
| 来源 | 价格 | 说明 |
|---|---|---|
| 粗查(URL拼接, 无mi_id) | ¥51 | 仅 sku2info 优惠前价; URL 无 upStreamPrice |
| 细查(带 mi_id 页) | 页面显示 ¥51; URL `upStreamPrice=42.79` | 显示价=粗查; 但 URL 带淘宝注入的上游/实付价 |
| 购物车(真实加购 API) | **到手 ¥42.79**(原价¥51, 省¥7.7) | platform_after=42.79 = 细查 upStreamPrice=4279 |

- **`upStreamPrice`(URL参数, 分为单位) = 购物车真实到手价** — 与加购后 platform_after 完全一致。
- **粗查读不到到手价**: 无 mi_id 页面只暴露标价/优惠前(¥51), 真实优惠(¥51→¥42.79)只在
  mi_id 链路(细查页 upStreamPrice / 购物车)暴露。
- **对 A2/选品含义**: 若要比"真实到手价"(而不仅是标价), 粗查不够 — 需 fine(mi_id)读
  upStreamPrice, 或走购物车 API。粗查只适合作"候选发现/标价排序", 到手价比对需 fine。
- 用户纠正: 之前误记"详情页触发验证码"实为残留页面 bug(已修复, 非详情页问题)。
- 购物车已用 remove 退回 PETG, 无残留。
